 ...
 * @details
 *   帧结构（13字节）：
 *   [AA] [55] [dx_L] [dx_H] [dy_L] [dy_H] [valid] [type] [phase_L] [phase_H] [rpm_L] [rpm_H] [chk]
 *
 *   字段说明：
 ...
 */
const FrameConfig_t g_config_esc2026 = {
    .name = "QD4310",
    .sof = {0xAA, 0x55},
    .field_num = 6,   //6个有效字段
    .fields = {
        ...
        {FIELD_TYPE,      1, 0, 1.0f},                          ///< type: uint8，原值
...
bool Vision_SelectConfig(const char *name)
{
    ...
    // 根据名称选择配置（在此处添加新配置）
    if (strcmp(name, "ESC2026") == 0) {
        new_cfg = &g_config_esc2026;
